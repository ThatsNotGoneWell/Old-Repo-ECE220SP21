...
//    You may need to change the return statement below
    ...
    if (sscanf (seed_str, "%d%1s", &seed, post) != 1) {
      // your code here
      printf("set seed: invalid seed\n"); // if the the input is not one interget, then invalid
      return 0;
    }
 ...
 */
void start_game () {
  int first, second, third, fourth; // initialize int
  first = rand()%8;	// using rand and modulus to randomly select a number from 0-7 to integers
  second = rand()%8;
  third = rand()%8;
  fourth = rand()%8;
  
  strcpy(solutions[0], pool[first]); // copy the value to solutions
  strcpy(solutions[1], pool[second]);
  strcpy(solutions[2], pool[third]);
  strcpy(solutions[3], pool[fourth]);
...
int make_guess (const char guess_str[]) {
  ...
  char guesses[4][10]; // store user's guesses
  char post[20]; // store extra garbage strings
  ...
  // Run if guesses are valid
  int mark_key[]={0, 0, 0, 0};
  ...
  for (i=0; i<4; i++) {
    if (strcmp(guesses [i], solutions[i]) == 0) {
      mark_key[i] = 1;
      mark_guess[i] = 1;
      perfect_match += 1; // increment # of perfect match
  ...
  for (k=0; k<4; k++) {
    if (mark_guess[k] == 0){
      for (j=0; j<4; j++){
		if (strcmp(guesses[k], solutions[j]) == 0 && mark_key[j] == 0) {
	  		mark_key[j] = 1;
	 	 	mark_guess[k] = 1;
	  		misplaced_match += 1; // increment # of misplaced match
